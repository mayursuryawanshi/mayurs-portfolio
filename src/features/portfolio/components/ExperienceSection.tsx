...
const experiences = [
  {
    ...
    skills: ["React.js", "TypeScript", "Python", "Django", "AI/ML", "Healthcare"],
    color: "bg-neo-cyan",
    current: true,
  },
  {
  ...
  },
  {
    ...
    skills: ["TypeScript", "React.js", "NestJS", "Docker", "Terraform", "shadcn/ui"],
    color: "bg-neo-orange",
  },
  {
...
const ExperienceSection = () => {
  ...
  return (
    <section id="experience" className="py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-neo-cream pattern-grid-diagonal border-t-4 border-foreground">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          <div className="inline-flex items-center neo-tag bg-neo-cyan px-4 py-2 mb-6">
            ...
            <span className="font-display text-xs tracking-[0.15em] uppercase">Career Path</span>
          </div>
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                <div
                  ...
                  ref={cardRef.ref}
                  className={`relative pl-0 sm:pl-20 md:pl-24 scroll-fade-in-up ${cardRef.isVisible ? "visible" : ""}`}
                  style={{ transitionDelay: `${index * 0.1}s` }}
                >
                  {/* Timeline dot */}
                  <div className="absolute left-4 md:left-6 top-6 hidden sm:flex">
                    <div className={`w-5 h-5 neo-border ${exp.current ? "bg-neo-yellow" : "bg-white"} neo-shadow-sm z-10 ${exp.current ? "animate-pulse-scale" : ""}`} />
                  </div>

                  {/* Card */}
                  <div className="relative">
                    <div className="absolute inset-0 neo-card bg-foreground translate-x-1.5 translate-y-1.5 hidden sm:block" />
                    <div className={`neo-card bg-white p-6 md:p-8 relative z-10`}>
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-4">
                        <div>
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            <h3 className="font-display text-xl md:text-2xl">{exp.title}</h3>
                            {exp.current && (
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className={`neo-border ${exp.color} px-3 py-1 font-display text-sm`}>
                              {exp.company}
                            </span>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.skills.map((skill) => (
                          <span key={skill} className="neo-border px-3 py-1 text-xs font-display uppercase tracking-wide bg-neo-cream neo-shadow-sm">
                            {skill}
                          </span>
                        ))}
                      </div>
      ...
      {/* Floating Shapes */}
      <ScrollShape zone="top" side="right" size="w-14 h-14" shape="shape-stripes-pink" animation="scroll-spin-in" postAnimation="post-float" delay="0.1s" />
      ...
      <ScrollShape zone="mid" side="right" size="w-16 h-16" shape="shape-checkerboard" animation="scroll-slide-rotate-right" postAnimation="post-wiggle" delay="0.3s" />
      <ScrollShape zone="top" side="left" size="w-16 h-16" shape="shape-grid" animation="scroll-drop-in" delay="0.35s" hideBelow="lg" />
      <ScrollShape zone="bottom" side="right" size="w-14 h-14" shape="shape-rings" animation="scroll-flip-in" rounded delay="0.4s" />
      <ScrollShape zone="mid" side="left" size="w-14 h-14" shape="shape-stripes-teal" animation="scroll-scale-up" delay="0.45s" hideBelow="lg" />
      <ScrollShape zone="bottom" side="left" size="w-18 h-18" shape="shape-stripes-pink" animation="scroll-zoom-rotate" postAnimation="post-float" rounded delay="0.5s" />
      <ScrollShape zone="top" side="right" size="w-16 h-16 xl:w-18 xl:h-18" shape="shape-dots" animation="scroll-bounce-in" postAnimation="post-pulse" delay="0.25s" />
    </section>
  );
};
